...
CREDENTIALS_JSON = os.getenv(
    "CREDENTIALS_JSON", "/home/rafa1215/consensus-project/credentials.json"
)
...
def fetch_off_by_barcode(
    barcode: str,
) -> Tuple[Optional[str], Optional[str], Optional[float]]:
    try:
        url = f"https://world.openfoodfacts.org/api/v2/product/{barcode}.json"
        r = requests.get(
            url, headers={"User-Agent": USER_AGENT}, timeout=OPENFOODFACTS_TIMEOUT
        )
        if r.status_code != 200:
            return None, None, None
        product = (r.json() or {}).get("product") or {}
        name = product.get("product_name") or product.get("product_name_en")
        ingredients_text = product.get("ingredients_text") or product.get(
            "ingredients_text_en"
        )
...
def _authorize():
    ...
    try:
        ...
        if (
            "Unexpected credentials type" in str(e)
            or "service_account" in str(e).lower()
        ):
            from gspread import oauth

            gc = oauth(
                credentials_filename=CREDENTIALS_JSON,
                authorized_user_filename=TOKEN_JSON,
            )
...
def open_sheet():
    ...
    try:
        ...
        logging.info(
            f"Worksheet '{WORKSHEET_NAME}' not found; using first sheet: '{ws.title}'"
        )
    ...
    if not values:
        ...
        if LAST_RUN_CELL:
            try:
                ws.update_acell(
                    LAST_RUN_CELL,
                    f"Last keto tag run: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                )
            except Exception:
                pass
        return
        ...
        try:
            ...
            if not name or not desc or tag is None:
                ...
                if not name and off_name:
                    ws.update_cell(idx, COL_NAME, off_name)
                    name = off_name
                if not desc and off_desc:
                    ws.update_cell(idx, COL_DESC, off_desc)
                    desc = off_desc
                if tag is None:
