...
def _log_voice_error(e: Exception) -> None:
    try:
        ...
        try:
            ...
            pass
        AGENTS_HEARTBEAT.mkdir(parents=True, exist_ok=True)
        with (AGENTS_HEARTBEAT / f"voice_errors_{date.today().isoformat()}.log").open(
            "a", encoding="utf-8"
        ) as f:
...
def _git_version() -> str:
    try:
        ...
        line = subprocess.check_output(
            ["git", "show", "-s", "--format=%cI %s", "HEAD"], cwd=ROOT, text=True
        ).strip()
        return f"{sha} {line}"
    except Exception:
        return "unknown"
...
def memory_absorb():
    ...
    try:
        ...
        commit = "unknown"
    return _voice_json(
        {"ok": True, "commit": commit, "ts": st["ts"], "hits_today": st["hits_today"]},
        200,
    )
...
def voice_last_absorb():
    ...
    text = (
        f"The latest GitHub memory absorb was at {ts} UTC. Absorbs today: {hits}."
        if ts
        else "I don't have a recorded GitHub memory absorb yet."
    )
...
def _summarize(records: list) -> str:
    try:
        ...
        n = len(records)
        keto = sum(
            1 for r in records if (r.get("class", "").lower().startswith("keto"))
        )
        slightly = sum(
            1 for r in records if (r.get("class", "").lower().startswith("slightly"))
        )
        nonfood = 0
        nf_re = re.compile(
            r"(tissue|towel|sanitizer|soap|detergent|shampoo|deodorant|lotion|trash bag)",
            re.I,
        )
        for r in records:
...
def _lookup(records: list, q: str) -> list:
    ...
    for r in reversed(records):
        hay = " ".join(
            [r.get("item", ""), r.get("details", ""), r.get("class", "")]
        ).lower()
...
def voice_barcode_lookup():
    ...
    if not q.strip():
        return _voice_json(
            {"ok": True, "text": "No query provided.", "count": 0, "items": []}, 200
        )
    hits = _lookup(recs, q)
    if not hits:
        return _voice_json(
            {"ok": True, "text": f"No matches for '{q}'.", "count": 0, "items": []}, 200
        )
    names = []
    for r in hits[:5]:
    ...
    return _voice_json(
        {"ok": True, "text": line, "count": len(hits), "items": hits}, 200
    )
...
def voice_barcode_probe():
    ...
    recs = _load_cache()
    return _voice_json(
        {"ok": True, "count": len(recs), "has_cache": bool(recs), "sample": recs[:2]},
        200,
    )
    ...
    try:
        rules = sorted(
            [
                ...
                if str(r.rule).startswith("/voice/")
            ]
        )
...
def _voice_global_errors(e):
    try:
        ...
        if str(path).startswith("/voice/"):
            ...
            return _voice_json(
                {"ok": False, "error": "server_error", "reason": reason}, 200
            )
        ...
        try:
            if str(request.path).startswith("/voice/"):
                return _voice_json(
                    {"ok": False, "error": "server_error", "reason": "handler_failed"},
                    200,
                )
        except Exception:
            pass
