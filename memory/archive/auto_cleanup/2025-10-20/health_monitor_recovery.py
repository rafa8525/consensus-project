...
MONITORED_LOGS = {
    ...
    "/home/rafa1215/consensus-project/memory/logs/watchdog/sms_simulation_log.txt": 6
    * 3600,
    "/home/rafa1215/consensus-project/memory/logs/watchdog/wsgi_restart_log.txt": 24
    * 3600,
}
...
def send_sms_alert(message):
    send_sms(body=message, from_=TWILIO_FROM_NUMBER, to=TWILIO_TO_NUMBER)
...
def check_and_recover():
    ...
    for filepath, max_age_sec in MONITORED_LOGS.items():
        ...
        if age > max_age_sec:
            alerts.append(
                f"❌ {os.path.basename(filepath)} not updated in last {age//3600:.0f} hours."
            )
            ...
            if cmd:
                try:
                    ...
                    logging.error(
                        f"Failed to execute recovery command for {filepath}: {e}"
                    )
            else:
