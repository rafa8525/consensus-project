...
def _rate_ok() -> bool:
    with LOCK:
        ...
        if RATE_STATE.exists():
            try:
                data = json.loads(RATE_STATE.read_text() or "{}")
            except Exception:
                data = {}
...
def send_sms(
    client, *, to: str, body: str, template: str = "", **kwargs
) -> Dict[str, Any]:
    idem = _hash_idempotency(to, body, template)
    if SILENCE:
        _log(
            LOG_FILE,
            f"BLOCKED(silence) to={to} idem={idem} body={body[:160]!r} kwargs={{{', '.join(kwargs)}}}",
        )
        return {"status": "blocked", "reason": "silence", "to": to, "idem": idem}
    if not ENABLE_SEND:
        _log(
            LOG_FILE,
            f"BLOCKED(enable_flag_missing) to={to} idem={idem} body={body[:160]!r}",
        )
        return {
            "status": "blocked",
            "reason": "enable-flag-missing",
            "to": to,
            "idem": idem,
        }
    if not _rate_ok():
