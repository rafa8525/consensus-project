...
def run_agent_self_assessment():
    ...
    try:
        subprocess.run(
            ["git", "add", "memory/logs/agent_self_assessment.md"], check=True
        )
        subprocess.run(
            ["git", "commit", "-m", f"Automated self-assessment update {now}"],
            check=True,
        )
...
result = subprocess.run(
    ["python3", "push_existing_logs_only.py"], capture_output=True, text=True
)
...
zip_result = subprocess.run(
    ["python3", "zip_logs_and_push.py"], capture_output=True, text=True
)
...
reminder_result = subprocess.run(
    ["python3.10", "/home/rafa1215/consensus-project/reminder agent/reminder_agent.py"],
    capture_output=True,
    text=True,
)
...
try:
    ...
    subprocess.run(
        [
            ...
            "commit",
            "-m",
            f"Automated fitness/barcode/geofencing log updates {now}",
        ],
        check=True,
    )
    subprocess.run(["git", "push"], check=True)
...
try:
    ...
    subprocess.run(
        ["git", "commit", "-m", f"Automated fitness summary {now}"], check=True
    )
    subprocess.run(["git", "push"], check=True)
...
send_sms_alert(
    "✅ All AI Consensus logs synced. Daily tasks and health summary complete."
)  # <--- Twilio SMS alert
