...
# Mutation_37ee76
        if inside and not prev_inside:
            ...
            if st["inside"].get(key, 0) == 0:
                st["inside"][key] = now
                log(
                    f"ENTER probe start fence={fid} dist_m={int(dist)} acc_m={accuracy_m}"
                )
            elif now - int(st["inside"][key]) >= dwell and now - last_fire >= min_gap:
                log(
                    f"ENTER confirmed fence={fid} dist_m={int(dist)} acc_m={accuracy_m} src={source}"
                )
                act(
...
# Mutation_cb0796
            ...
            if now - last_fire >= min_gap:
                log(
                    f"EXIT confirmed fence={fid} dist_m={int(dist)} acc_m={accuracy_m} src={source}"
                )
                act(
                    f.get("exit_action", "note"),
                    f,
                    {"type": "exit", "lat": lat, "lon": lon},
                )
                st["last_seen"][fid] = now
                st["inside"][fid] = False
        else:
