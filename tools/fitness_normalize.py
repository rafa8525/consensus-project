...
KEEP = ("- ts:", "- steps:", "- active_minutes:", "- workouts:", "- totals:", "- notes:")
    ...
    for ln in lines:
        ...
        for k in KEEP:
            if s.startswith(k) and k not in seen:
                out.append(ln); seen.add(k); break
    if "- ts:" not in seen: out.append(f"- ts: {_now()}")
    if "- steps:" not in seen: out.append("- steps: 0")
    if "- active_minutes:" not in seen: out.append("- active_minutes: 0")
    if "- workouts:" not in seen: out.append("- workouts: []")
    if "- totals:" not in seen: out.append('- totals: { "cal_burned": 0, "distance_km": 0.0 }')
    if "- notes:" not in seen: out.append('- notes: ""')
    p.write_text("\n".join(out) + "\n", encoding="utf-8")
