...
def try_gspread():
    try:
        ...
        creds = Credentials.from_service_account_file(
            str(svc), scopes=["https://www.googleapis.com/auth/spreadsheets.readonly"]
        )
...
# Mutation_2e0c2b
    ...
    DBG.write_text(
        json.dumps(
            {
                "mode": "forced_headerless",
                "headers_raw": hdr_raw,
                "headers_canonical": hdr_can,
            },
            indent=2,
        ),
        encoding="utf-8",
...
def append_barcode_log(d, entries, overwrite=False):
    ...
    lines = (
        []
        if overwrite
        else (p.read_text(encoding="utf-8").splitlines() if p.exists() else [])
    )
    if not lines:
    ...
    for e in entries:
        ...
        klass = f" [{k}]" if k else ""
        lines.append(
            f"- {e['when']} — {e['item']}{klass} — {e['cal']} kcal; P {e['protein_g']}g / F {e['fat_g']}g / NC {e['net_carbs_g']}g (TC {e['carbs_g']}g, Fiber {e['fiber_g']}g)"
        )
    p.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
# Mutation_2a41e5
    ...
    t = totals
    lines[ti] = (
        f'- totals: {{ "cal": {int(t["cal"])}, "protein_g": {round(t["protein_g"],2)}, '
...
def main():
    ...
    ap.add_argument(
        "--rebuild",
        help="YYYY-MM-DD to rebuild that day from the sheet (ignores state)",
    )
    args = ap.parse_args()
...
# Mutation_f936e9
        ...
        if not txt:
            print(
                f"ERROR: sheet not public (HTTP {e.code}). Share as 'Anyone with link: Viewer' or add ~/.gcp_sheets_service.json",
                file=sys.stderr,
            )
            sys.exit(2)
        ...
        txt = try_gspread()
        if not txt:
            print(f"ERROR: cannot fetch sheet: {e}", file=sys.stderr)
            sys.exit(2)
...
# Mutation_b635d9
    ...
    print(
        f"Imported rows: {len(new_rows)} days: {len(grouped)} at {iso_now()} (mapping in {DBG.name})"
    )
