...
class KnowledgeBaseHandler(FileSystemEventHandler):
    ...
    def agent_collaboration_test(self):
        ...
        try:
            ...
            with open(KB_FILE, "a") as f:
                f.write(
                    f"[{now}] AGENT_B: Read Agent A's message and confirmed propagation.\n"
                )
            # Auto-sync the new updates
            subprocess.run(["git", "add", KB_FILE], cwd=PROJECT_ROOT, check=True)
            subprocess.run(
                ["git", "commit", "-m", f"Agent collaboration test at {now}"],
                cwd=PROJECT_ROOT,
                check=True,
            )
            subprocess.run(["git", "push"], cwd=PROJECT_ROOT, check=True)
...
if __name__ == "__main__":
    ...
    observer.start()
    print(
        f"Watching {KB_FILE} for changes. Auto-sync and agent collaboration test enabled."
    )
    try:
        while True:
            time.sleep(5)
