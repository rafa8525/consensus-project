...
# ---- CONFIG ----
SHEET_ID = (
    "1Xn9GB_GJtJmU1fI4kc7mdsvuz6xGzo2oFU8_YdkwGAQ"  # Change if you rename/move sheet
)
RANGE_NAME = "Sheet1!A2:G"  # Data rows (skip headers)
TIMEZONE = "America/Los_Angeles"  # Set to your timezone (e.g., 'America/Los_Angeles')
...
def send_sms(message):
    payload = {"message": message, "secret": WEBHOOK_SECRET}
    try:
...
def main():
    ...
    for idx, row in enumerate(rows):
        ...
        # Parse due time
        try:
            due_dt = tz.localize(datetime.datetime.strptime(due_time, "%Y-%m-%d %H:%M"))
        except Exception:
        ...
        if status.lower() == "pending":
            ...
            if last_notified.strip():
                try:
                    notified_dt = tz.localize(
                        datetime.datetime.strptime(last_notified, "%Y-%m-%d %H:%M")
                    )
                except Exception:
                    pass
    ...
    if updates:
        ...
        for rownum, ts in updates:
            data.append({"range": f"Sheet1!E{rownum}", "values": [[ts]]})
