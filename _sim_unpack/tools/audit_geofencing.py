...
def classify(line: str):
    ...
    if any(
        w in l
        for w in ["feature", "should", "need to", "please add", "want", "request"]
    ):
        return "feature"
    if any(
        w in l for w in ["test", "tried", "implemented", "enabled", "wired", "deployed"]
    ):
        return "attempt"
    return "other"
...
def main():
    ...
    if len(attempts) > 20:
        out_lines.append(f"- … {len(attempts)-20} more")
    ...
    if len(failures) > 20:
        out_lines.append(f"- … {len(failures)-20} more")
    ...
    if len(features) > 50:
        out_lines.append(f"- … {len(features)-50} more")
    ...
    (OUTD / "geofencing_audit.json").write_text(
        json.dumps(
            {"generated_at": ts, "hits": hits, "top_failure_patterns": top_fail},
            ensure_ascii=False,
            indent=2,
        ),
        encoding="utf-8",
