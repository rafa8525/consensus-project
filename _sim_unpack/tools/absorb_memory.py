...
IGNORE_DIRS = {".git", "__pycache__", "archive", "github_sync"}
IGNORE_PATTERNS = [
    r".*\.zip$",
    r".*\.gz$",
    r".*\.pdf$",
...
MAX_FILE_BYTES = (
    5_000_000  # 5MB content cap for keyword scan (index still records larger files)
)
...
def want(path: Path) -> bool:
    ...
    if any(p in IGNORE_DIRS for p in parts):
        return False
    s = str(rel)
    if "logs/heartbeat/full_memory_absorption.log" in s:
        return False
...
def main():
    ...
    for p in MEM.rglob("*"):
        ...
        item = {
            ...
            "sha1": sha1_file(
                p, limit=min(st.st_size, 2_000_000)
            ),  # partial hash for speed
        }
        # light-weight keywords for text-ish files
        if st.st_size <= MAX_FILE_BYTES and re.search(
            r"\.(txt|md|json|py|sh|log)$", rel, re.I
        ):
            txt = extract_text(p)
    ...
    (IDX_DIR / "search_index.json").write_text(
    ...
    with hb.open("a", encoding="utf-8") as f:
        f.write(
            f"[{time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())}] indexed={len(manifest)} elapsed={elapsed}s\n"
        )
