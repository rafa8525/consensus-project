...
def run_git_sync():
    try:
        ...
        os.system(f"echo '\n--- git status ---' >> {log_path}")
        os.system(
            f"cd /home/rafa1215/consensus-project && git status >> {log_path} 2>&1"
        )

        os.system(f"echo '\n--- git add ---' >> {log_path}")
        os.system(
            f"cd /home/rafa1215/consensus-project && git add memory/ >> {log_path} 2>&1"
        )

        os.system(f"echo '\n--- git commit ---' >> {log_path}")
        os.system(
            f"cd /home/rafa1215/consensus-project && git commit -m 'Voice-triggered sync' >> {log_path} 2>&1"
        )
