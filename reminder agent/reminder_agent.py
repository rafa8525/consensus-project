...
# ---- CONFIG ----
SHEET_ID = (
    "1Xn8GB_GITJmU1f4kcJ7mdsvuz6zGoo2oFU8_YxkwGAQ"  # Your sheet ID (from your link)
)
...
def get_creds():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file("/home/rafa1215/.secrets/google/credentials.json", SCOPES)
...
def main():
    ...
    for idx, row in enumerate(rows):
        ...
        try:
            due_dt = tz.localize(
                datetime.datetime.strptime(due_time.strip(), "%Y-%m-%d %H:%M")
            )
        except Exception:
