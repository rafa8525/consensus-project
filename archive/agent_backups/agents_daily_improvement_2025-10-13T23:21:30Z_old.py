...
def synthesize(day: str):
    items = []
    items += [
        f"Systematize: {b}" for b in bullets_from_md(AG / f"lessons_learned_{day}.md")
    ]
    items += [
        f"Codify docs: {b}" for b in bullets_from_md(AG / f"knowledge_shared_{day}.md")
    ]
    ...
    if vt.exists():
        items.append(
            f"Instrument /voice_trigger latency + status; today={count_lines(vt)} hits."
        )
