    ...
    if HB.exists():
        ...
        for line in reversed(lines):
            ...
            if m2:
                ...
                if IDXF.exists():
                    try:
                        approx = json.loads(IDXF.read_text(encoding="utf-8")).get(
                            "total_files"
                        )
                    except Exception:
                        pass
                ...
                if brief:
                    ...
                    print(
                        f"Absorbed {human_ago_from_epoch(epoch)} ({fmt_ts(epoch, tz_mode, tz_name)}{extra})."
                    )
                else:
                    where = fmt_ts(epoch, tz_mode, tz_name)
                    if approx is not None:
                        print(
                            f"Last absorption was {where} ({human_ago_from_epoch(epoch)}), indexed ~{approx} files."
                        )
                    else:
                        print(
                            f"Last absorption was {where} ({human_ago_from_epoch(epoch)})."
                        )
                return
        ...
        try:
            ...
            if gen:
                ...
                if brief:
                    ...
                    print(
                        f"Last absorption (from index) was {fmt_ts(epoch, tz_mode, tz_name)} ({human_ago_from_epoch(epoch)}), indexed {idx} files."
                    )
                return
        except Exception:
            pass
...
def last_github_update(tz_mode: str, tz_name: str, brief: bool):
    ...
    if SYNC.exists():
        lines = [
            l
            for l in SYNC.read_text(encoding="utf-8", errors="ignore").splitlines()
            if l.strip()
        ]
        for line in reversed(lines):
            if line.startswith("[") and "] " in line:
                ...
                if re.fullmatch(r"[0-9a-f]{7,40}", h):
                    ...
                    if brief:
                        ...
                        print(
                            f"Last GitHub update to memory/ was commit {h[:9]} at {fmt_ts(epoch, tz_mode, tz_name)}."
                        )
                    return
